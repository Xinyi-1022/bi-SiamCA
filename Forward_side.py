...
# We give the three validate trials we choosed in the following table. table[test, validate, subject]
    ...
    for test_trial in range(0, 6):
        for subj in range(1, 36):
            for validate_num in range(0,3):
                ...
                save_dir = './50TimeWindow_positive/subj' + str(subj) + '_test' + str(test_run[0]) + '_validate' + str(validate_trial[0]) +'.pth'
                if save_model:
                ...
                for epoch in range(n_epochs):
                    ...
                    for split in ('train', 'validate'):
                        ...
                        for i, batch in enumerate(dataset[split]):
                            signal = batch[0].to(device).float()
                            reference = batch[1].to(device).float()
